...
func (s *BundleAPI) CallBundle(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	...
	if args.RichAddress != nil && len(args.RichAddress) > 0 {
		...
		for _, richAddr := range args.RichAddress {
			state.SetBalance(common.HexToAddress(*richAddr), GiveEtherNew.ToBig())
	...
	difficulty := big.NewInt(2) // always in-turn
...
func ApplyTransactionWithACLResult(
	...
	precompiles := vm.ActivePrecompiles(config.Rules(header.Number, header.Time))
	...
	// trace용 config
	traceConfig := vm.Config{Tracer: tracer, NoBaseFee: true}
	// Create a new context to be used in the EVM environment
	blockContext := core.NewEVMBlockContext(header, bc, author)
	txContext := core.NewEVMTxContext(msg)
